...
2026-09-24 時点で、本番は `a5af9d6` のコードで稼働している。本番DBに適用済みのマイグレーションは10本で、ドロップの導入の4本（→3章）は 2026-09-25 時点でまだ出していない。列を消す1本（`drop_well_record_columns`）だけは元に戻せないので、別のブランチ `feature/plate-drops-cleanup` に分けてある。残りの3本がある `feature/plate-drops` を先に出し、本番で確かめてから cleanup を出す。出し方の順番と、出す前に本番で確かめること（`PlateType` の `wellCount` が24と96だけか、など）は計画書 `docs/plans/2026-09-24-plate-drops.md` の「出し方」と各 Phase の完了条件にある。
